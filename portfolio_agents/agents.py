...
def propose_portfolio_node(state):
    ...
    try:
        ...
        llm_reasoning = proposal.get('reasoning') or proposal.get('rationale')
        
        if isinstance(proposal, dict):
            # FIX: Dynamically fetch whichever key the LLM chose to use
            raw_allocation = proposal.get('portfolio_allocation') or proposal.get('allocation')
            
            # Scenario A: LLM gave us a list of dicts
            if isinstance(raw_allocation, list):
                ...
                for item in raw_allocation:
                    if isinstance(item, dict) and 'ticker' in item and 'weight' in item:
                        ...
                        proposed_portfolio[ticker] = float(item['weight'])
                        if 'reasoning' in item:
                            list_reasonings.append(f"{ticker}: {item['reasoning']}")
                
                if not llm_reasoning and list_reasonings:
                    llm_reasoning = "\n".join(list_reasonings)
                    
            # Scenario B: LLM followed the flat dictionary structure
            elif isinstance(raw_allocation, dict):
                proposed_portfolio = raw_allocation
            
            # Scenario C: LLM returned the allocation flat at the root level
            elif all(isinstance(k, str) and isinstance(v, (float, int)) for k, v in proposal.items()):
                proposed_portfolio = proposal
            else:
                raise ValueError("LLM output structure could not be parsed into an asset allocation.")
        else:
        ...
        # Filter and renormalize weights to sum to 1.0
        ...
        return {
            ...
            "llm_commentary": str(llm_reasoning) if llm_reasoning else None,
